...
public class WhitePiece extends Piece {
	...
	private int[] SINGLE_MOVE_OFFSETS = { -6, -7, -8 };
	private int[] ATTACK_MOVE_OFFSETS = { 2, 12, 14, 16, -2, -12, -14, -16 };

	public WhitePiece(int piecePosition) {
		super(piecePosition);
	}
	...
	public Piece movePiece(Move move) {
		return new WhitePiece(move.getDestinationCoordinate());
	}
		...
		for (int offset : ATTACK_MOVE_OFFSETS) {
			...
			if (BoardUtils.isValidTileCoordinate(targetId)) {
				...
				if (pieceInBetween != null) {
					...
					if (BoardUtils.SEVENTH_COLUMN[piecePosition] && (offset == -12 || offset == 2 || offset == 16)) {
						continue;
					}

					if (BoardUtils.SIXTH_COLUMN[piecePosition] && (offset == -12 || offset == 2 || offset == 16)) {
						continue;
					}

					if (BoardUtils.SECOND_COLUMN[piecePosition] && (offset == -2 || offset == -16 || offset == 12)) {
						continue;
					}

					if (BoardUtils.FIRST_COLUMN[piecePosition] && (offset == -2 || offset == -16 || offset == 12)) {
						continue;
					}

					if ((offset == 16 || offset == 14 || offset == 12 || offset == 2 || offset == -16 || offset == -14
							|| offset == -12 || offset == -2) && !targetTile.isTileOccupied()
							&& pieceInBetween.getPieceAlliance().isBlack()) {
	...
	public void calculateLegalStandardMoves(Board board) {
		...
		for (int offset : SINGLE_MOVE_OFFSETS) {
			...
			if (BoardUtils.isValidTileCoordinate(targetId)) {
				if (BoardUtils.SEVENTH_COLUMN[this.piecePosition] && offset == -6) {
					continue;
				}
				if (BoardUtils.FIRST_COLUMN[this.piecePosition] && offset == -8) {
					continue;
				}
